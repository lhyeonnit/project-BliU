...
import 'package:BliU/screen/_non/non_mypage/bottom/component/non_inquiry_product_detail.dart';
...
class NonServiceInquiryProduct extends ConsumerStatefulWidget {
  const NonServiceInquiryProduct({super.key});

  @override
  _NonServiceInquiryProductState createState() => _NonServiceInquiryProductState();
}

class _NonServiceInquiryProductState extends ConsumerState<NonServiceInquiryProduct> {
  int currentPage = 1;
                          ...
                          onTap: () {
                            ...
                            if (qtIdx != null) {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => NonInquiryProductDetail(
                                    qtIdx: qtIdx,
                                  ),
                                ),
                              );
  ...
  void _getList(bool isNew) {
    if (isNew) {
      currentPage = 1;
      ref
          .read(serviceInquiryProductModelProvider)
          ?.qnaListResponseDTO
          ?.list
          ?.clear();
    }
    ...
    SharedPreferencesManager.getInstance().then((pref) {
      ...
      ref
          .read(serviceInquiryProductModelProvider.notifier)
          .getList(requestData);
    });
  }
}
