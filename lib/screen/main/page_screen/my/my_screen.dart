...
class MyScreenState extends ConsumerState<MyScreen> {
  ...
  void _viewWillAppear(BuildContext context) async {
    ...
    if (mtIdx != null && mtIdx.isNotEmpty) {
      ...
      await ref.read(myViewModelProvider.notifier).getMy(requestData);
    } else {
  ...
  void _logout() async {
    ...
    Map<String, dynamic> requestData = {
      'mt_idx': mtIdx,
      'mt_app_token': pref.getToken(),
    };

    final result = await ref.read(myViewModelProvider.notifier).logout(requestData);
    if (result) {
      await pref.logOut(); // 저장된 모든 데이터를 삭제
      ref.read(myViewModelProvider.notifier).resetData();
      ...
      if(!mounted) return;
      Utils.getInstance().showSnackBar(context, '로그아웃에 실패했습니다.');
    }
  }

  void _setData() {
    final model = ref.read(myViewModelProvider);
    ...
    if (memberInfoResponseDTO != null) {
      ...
      if (childCk == "N" && context.mounted) {
        Navigator.pushNamed(context, '/recommend_info');
      }
    } else {
      setState(() {
        userId = '';
