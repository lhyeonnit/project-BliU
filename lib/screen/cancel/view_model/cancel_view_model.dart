...
import 'package:BliU/dto/order_cancel_info_response_dto.dart';
...
class CancelViewModel extends StateNotifier<CancelModel?> {
  ...
  Future<OrderCancelInfoResponseDto?> getOrderCancelInfo(Map<String, dynamic> requestData) async {
    try {
      final response = await repository.reqPost(url: Constant.apiMyPageOrderCancelInfoUrl, data: requestData);
      if (response != null) {
        if (response.statusCode == 200) {
          Map<String, dynamic> responseData = response.data;
          OrderCancelInfoResponseDto orderCancelInfoResponseDto = OrderCancelInfoResponseDto.fromJson(responseData);
